...
class ElasticSearch {
	...
	constructor(config = {}) {
		...
		try {
			const hostProtocol = `${new URL(this.config.host).protocol}//`; // throws if the URL doesn't have a protocol
			this.config.protocol = hostProtocol;
		} catch(err) {
			// do nothing...
		}

		this.config.host = this.config.host.replace(this.config.protocol, ''); // Avoid protocol duplication
	}
	...
	*/
	...
	_validateModel(model) {
		...
		if(typeof model.constructor.sortableFields !== 'object' || Array.isArray(model.constructor.sortableFields)) {
