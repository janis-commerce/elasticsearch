	...
	*/
		...
		if(!this.client) {
			try {
				this.client = new elasticsearch.Client({
					node: `${this.config.protocol}${this.userPrefix}${this.config.host}:${this.config.port}`,
	...
	/* async buildIndex(model) {
		...
		try {
			...
			const exists = await client.indices.exists({ index: this.config.database });

			if(!exists.body)
				await client.indices.create({ index: this.config.database });

			await client.indices.close({ index: this.config.database });
			...
			await client.indices.open({ index: this.config.database });
			// return JSON.stringify(this.getMappingsFromModel(model), null, 2);
	...
	getMappingsFromModel(model) {
		...
		const parsedMappings = { [model.constructor.table]: { properties: {} } };
			...
			for(const field of model.constructor.fields) {
				...
				if(typeof field === 'object' && !Array.isArray(field)) {
					Object.entries(field).forEach(([key, value]) => {
						parsedMappings[model.constructor.table].properties[key] = { type: value };
					});
					continue;
				}

				parsedMappings[model.constructor.table].properties[field] = {
					type: 'keyword'
				};
			}
	...
	parseItemsForBulkUpsert(items) {
		...
		for(const item of items)
			...
			parsedItems +=	`{ "update": { "_id": ${item.id}, "_type": "_doc"} }\n{ "doc": ${JSON.stringify({ ...item, lastModified: new Date() })}, "upsert": ${JSON.stringify({ ...item, dateCreated: new Date() })} }\n`;
	...
	async multiSave(model, items) {
		...
		console.log(this.parseItemsForBulkUpsert(items));
